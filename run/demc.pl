...
my ($cmd, $path_exe, $path_in, $path_out, $genome, $fasta, $binary);
...
$cmd= "$path_exe/pwm_thr_err/pwm_thr_err ${path_in}${motif_base}${i}${motif_ext} ${path_out}${motif_base}${i}${pwm_ext} ${genome_prom} ${path_out}${motif_base}${i}${dist_ext} ${binary} 0.002 0.0000005 ${pwm_log} 0.00002 0";
print "$cmd\n";
system $cmd;
}

$cmd= "$path_exe/denovo/denovo ${path_in}${fasta} ${binary} ${n_motifs} ${spacer_min} ${spacer_max} ${pvalue_ce} ${log_pvalue_bonf} ${asym_rat}";
print "$cmd\n";
system $cmd;
