...
struct combi {
	...
	//	void mem_out(void);	
	int fprintf_all(char* file, char* files, int mot1, char* motif1, int mot2, char* motif2, int len_a, int len_p, int len_sp, char* mode);
};
int combi::fprintf_all(char* file, char* files, int mot1, char* motif1, int mot2, char* motif2, int len_a, int len_p, int len_sp, char* mode)
{
	char head[6][12];
...
{
	int i, j, k, m, mot;
	char file_fasta[ARGLEN], partner_db[ARGLEN];
	char*** seq;// peaks
	...
	if (argc != 9)
	{
		fprintf(stderr, "Error: %s ", argv[0]);//1int thresh_num_min 2int thresh_num_max
		fprintf(stderr, "1input fasta 2input binary files pwm_thresholds 3no. of motifs");
		fprintf(stderr, " 4int spacer_min 5int spacer_max 6double pvalue_thr 7double -log10[p-value]_thr 8double asymmetry_ratio(-log10(ERR)) in CE\n");//9char mot_anchor 
		return -1;
	}
	for (i = 1; i < argc; i++)
	{
	...
	strcpy(partner_db, argv[2]); //h12hs, h12mm
	int n_motifs = atoi(argv[3]);// no. of top-scoring matrices
	int shift_min = atoi(argv[4]); // minimal spacer length
	int shift_max = atoi(argv[5]); // upper bound of spacer length
	double pvalue = atof(argv[6]); //expected recogntion rate
	double bonf_user = atof(argv[7]);
	double fold_asy = log10(atof(argv[8]));//threshold for log10(frp) fold asymmentry
	{
...
//	int mot_anchor = 0;// 0 = pwm from file >0 pwm from pre-computed database
	...
	strcpy(file_pval[0], "fisher_any_mot_");
	strcpy(file_pval[1], "fisher_full_mot_");
	strcpy(file_pval[2], "fisher_part_mot_");
	strcpy(file_pval[3], "fisher_over_mot_");
	strcpy(file_pval[4], "fisher_spac_mot_");
	strcpy(file_pval_table, "out_pval");
	char **name;
	{
		char name0[]="motif";
		int m0 = strlen(name0);
		name = new char* [n_motifs];
		if (name == NULL) { puts("Out of memory..."); exit(1); }
		for (j = 0; j < n_motifs; j++)
		{
			name[j] = new char[20];
			if (name[j] == NULL) { puts("Out of memory..."); exit(1); }
		}
		char buf[10];
		for (j = 0; j < n_motifs; j++)
		{
	...
	double **thr;
	thr = new double* [n_motifs];
	if (thr == NULL) { puts("Out of memory..."); exit(1); }
	for (i = 0; i < n_motifs; i++)
	{
		thr[i] = new double[NUM_THR];
		if (thr[i] == NULL) { fprintf(stderr, "Error: Out of memory..."); return -1; }
	}
	profile *real_one, *rand_one, * rand_hom_one;
	real_one = new profile[n_motifs];
	if(real_one == NULL) { puts("Out of memory..."); exit(1); }
	rand_one = new profile[n_motifs];
	if (rand_one == NULL) { puts("Out of memory..."); exit(1); }
	rand_hom_one = new profile[n_motifs];
	if (rand_hom_one == NULL) { puts("Out of memory..."); exit(1); }
	combi hist_obs_one, hist_exp_one;
	matrices *matrix;
	matrix = new matrices[n_motifs];
	if (matrix == NULL) { puts("Out of memory..."); exit(1); }
	//for real
	for (j = 0; j < n_motifs; j++)
	{
	...
	for (j = 0; j < n_motifs; j++)
	{
		rand_hom_one[j].nseq = nseq_rand;
		rand_hom_one[j].nam = 1;
		int ini = rand_hom_one[j].mem_in_nsit();
		if (ini == -1) { fprintf(stderr, "Error: Not enough memory...\n"); return -1; }
	}
	int* peak_len_rand;
	...
	for (i = 0; i < n_motifs1; i++)for (j = 0; j < n_motifs1 - i; j++)sims[i][j] = 1;
	...
	for (mot = 0; mot < n_motifs; mot++)
	{
		...
		int len_partner4 = len_partner * 4;
		fread(pfm, sizeof(double), len_partner4, in_pwm);
		fread(pwm, sizeof(double), len_partner4, in_pwm);
		int nthr_dist_all;
		fread(&nthr_dist_all, sizeof(int), 1, in_pwm);
		fread(thr_all[mot], sizeof(double), nthr_dist_all, in_pwm);
		fread(fpr_all[mot], sizeof(double), nthr_dist_all, in_pwm);
		if (mot != n_motifs1)motifp[mot + 1].p = ftell(in_pwm);
		motifp[mot].len = len_partner;
		int nthr_dist = 0;
		...
		motifp[mot].nth = nthr_dist;
		matrix[mot].mem_in(len_partner);
		for (i = 0; i < len_partner; i++)for (j = 0; j < OLIGNUM; j++)matrix[mot].wei[i][j] = pwm[i][j];
		for (i = 0; i < len_partner; i++)for (j = 0; j < OLIGNUM; j++)matrix[mot].fre[i][j] = pfm[i][j];		
		double min = 0, raz = 0;
		PWMScore(min, raz, len_partner, pwm);
		matrix[mot].norm();
		double fpr_select_o[NUM_THR];
		int index[NUM_THR];
		{
			int stfp = select_thresholds_from_pvalues(nthr_dist, thr_all[mot], fpr_all[mot], fpr_select_i, fpr_select_o, thr[mot], index);
			if (stfp == -1)
			{
		...
		for (i = 0; i < nseq_real; i++)
		{
			for (k = 0; k < real_one[mot].nsit[i]; k++)
			{
				...
				for (j = inx1; j >= inx2; j--)
				{
					if (thr_all[mot][j + 1] <= sco && thr_all[mot][j] >= sco)
					{
						pv_sc = fpr_all[mot][j];
						break;
					}
		...
		for (j = 0; j < NUM_THR; j++)
		{			
			fprintf(out_stat, "Partner %d", mot);
			fprintf(out_stat, "\t%d\t%f\t", j + 1, thr[mot][j]);
	...
	{
		int len_anchor = motifp[mot1].len;
		real_one[mot1].mot = mot1;
		//partner
		for (mot2 = mot1; mot2 < n_motifs; mot2++)
		{			
			//anchor					
			int cop = real_one[mot1].get_copy_rand(&rand_hom_one[mot1], height_permut);
			if (cop == -1)
			{
				fprintf(stderr, "Error: Rand Copy error Mot %d Thr One\n", mot1);
				return -1;
			}
			cop = real_one[mot2].get_copy_rand(&rand_one[mot2], height_permut);
			if (cop == -1)
			{
				fprintf(stderr, "Error: Rand Copy error Mot %d Thr One\n", mot);
				return -1;
			...
			int len_partner = motifp[mot2].len;
			for (m = 0; m < nseq_real; m++)thr_err_real[m] = 0;
			rand_one[mot2].mot = mot2;
			rand_hom_one[mot1].mot = mot1;
			int throwp = throw_predictions(peak_len_rand, &rand_hom_one[mot1], &rand_one[mot2], len_anchor, len_partner, 0, thr_err_real, nseq_real, nseq_rand, seq[0], height_permut, file_err);
			if (throwp == -1)
			{
			...
			int proj = projoin(xrand, name[mot1], rand_hom_one[mot1], rand_one[mot2], shift_min, shift_max, len_anchor, len_partner, thr_err_rand, nseq_rand, seq, &expected, &hist_exp_one, peak_len_rand, &rand_plot, nseq_two_sites_rand, fold_asy);
			if (proj == -1)
			{
				fprintf(stderr, "Error: Projoin Rand error Anc 0 Par %d\n", mot);
				return -1;
			}
			proj = projoin(xreal, name[mot1], real_one[mot1], real_one[mot2], shift_min, shift_max, len_anchor, len_partner, thr_err_real, nseq_real, seq, &observed, &hist_obs_one, peak_len_real, &real_plot, nseq_two_sites_real, fold_asy);
			if (proj == -1)
			{
			...
			}
			{
				...
				fprintf(out_proj, "MotA\t%d\tMotP\t%d\tRealNseqBothMot\t%d\tRandNseqBothMot\t%d\tNseqReal\t%d\tNseqRand\t%d\tNCE_any\t%d\tNCE_overlap\t%d\tNCE_spacer\t%d\n", mot1+1, mot2+1, nseq_two_sites_real, nseq_two_sites_rand, nseq_real, nseq_rand, observed.sit.any, observed.sit.overlap, observed.sit.spacer);
				fclose(out_proj);
			}
			...
			strcat(file_hist_one_spacer, "_spacer");			
			hist_obs_one.fprintf_all(file_hist, file_hist_spacer, mot1, name[mot1], mot2, name[mot2], len_anchor, len_partner, shift_max, modea);
			//if (mot_p == 0)
			hist_obs_one.fprintf_all(file_hist_one, file_hist_one_spacer, mot1, name[mot1], mot2, name[mot2], len_anchor, len_partner, shift_max, modea); //modew
			hist_exp_one.fprintf_all(file_hist_rand, file_hist_rand_spacer, mot1, name[mot1], mot2, name[mot2], len_anchor, len_partner, shift_max, modea);
			real_plot.sum();
			rand_plot.sum();
			//if (mot_p != 0)
			{
			...
			}
			{
				...
				{
					fprintf(out_pval_table, "%d\t%d", mot1 + 1, mot2 + 1);
				}
				for (i = 1; i < 5; i++)
				{
				...
				double pval_sim[2] = { 1, 1 };
				{
					...
					if (mot1 != mot2)
					{
						pvalue_similarity_tot = pfm_similarity(&matrix[mot1], &matrix[mot2], s_granul, s_overlap_min, s_ncycle_small, s_ncycle_large, pval_sim);
				...
				fclose(out_pval_table);
				rand_one[mot2].mem_out_sta();
				rand_one[mot2].mem_out_cep();
				rand_one[mot2].mem_out_cel();
				rand_one[mot2].mem_out_pv();
				for (i = 0; i < nseq_rand; i++)rand_one[mot2].nsit[i] = 0;
				rand_hom_one[mot1].mem_out_sta();
				rand_hom_one[mot1].mem_out_cep();
				rand_hom_one[mot1].mem_out_cel();
				rand_hom_one[mot1].mem_out_pv();
				for (i = 0; i < nseq_rand; i++)rand_hom_one[mot1].nsit[i] = 0;
			}
		}				
	}
	real_plot.mem_out();
	...
	for (mot = 0; mot < n_motifs; mot++)matrix[mot].mem_out(matrix[mot].len);
	for (k = 0; k < 2; k++)
	{
		for (i = 0; i < nseq_real; i++)
		{
