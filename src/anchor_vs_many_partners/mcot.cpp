...
int main(int argc, char *argv[])
{
	...
	{	
		...
		{			
			...
			int stfp=select_thresholds_from_pvalues(n_thr_touzet,thr_touzet,fp_rate,pvalue,pvalue_mult,fpr_select,thr,index);
			if (stfp == -1)
			{
				printf("Too bad input matrix of %d motif\n", mot);
				return -1;
			}
			for (i = 0; i < n_thr_touzet; i++)
			{
...
//			delete [] fp_rate;							
		...
		{					
			if(strstr(partner_db,"hs_core") !=NULL)
			{
				...
				if(mem==-1){puts("Not enough memory...");	exit(1);}
			}
			else
			{
				if(strstr(partner_db,"mm_core") !=NULL)
			...
			n_thr_touzet=motifs.nthr;
			fp_rate=new double [n_thr_touzet];
			if(fp_rate==NULL){puts("Out of memory...");return -1;}
			for(j=0;j<n_thr_touzet;j++)
			{
				fp_rate[j]=motifs.fpr[j];
				thr_touzet[j]=motifs.thr[j];
			}
			motifs.mem_out();
			memset(name_partner,'\0',sizeof(name_partner));
...
//int pwm_rec0(matrices *mat, double thr, int len_pro, int nseq_pro, char ***seq, profile *real)  count all sites
		...
		for(i=0;i<nseq_real;i++)
		{
			for(k=0;k<real_one[ap].nsit[i];k++)
			{
				...
				for(j=inx1;j>=inx2;j--)
				{
					if(thr_touzet[j+1]<=sco && thr_touzet[j]>=sco)
					{
						pv_sc=fp_rate[j];
						break;
					}
