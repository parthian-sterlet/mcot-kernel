...
//		if((n+1)%height==0){over=0;tot=0;}
	...
	//				printf("%d%c ",anc->sta[n][j],anc->cep[n][j]);
		...
		{		
			...
			if(clust_len[i]==NULL ){puts("Error: Out of memory...");return -1;}
			nsit_clust[i] = new int[n_clust[i]];
			if(nsit_clust[i]==NULL ){puts("Error: Out of memory...");return -1;}
			int clust_cur=0;
			nsit_clust[i][0]=1;
		...
		if(clust_seq==NULL){puts("Error: Out of memory...");return -1;}
		for(i=0;i<2;i++)
		{
			clust_seq[i] = new char*[n_clust[i]];
			if(clust_seq[i]==NULL ){puts("Error: Out of memory...");return -1;}
			int nsit_cur=0;
			for(j=0;j<n_clust[i];j++)
			{
				clust_seq[i][j] = new char[clust_len[i][j]+1];
				if(clust_seq[i][j]==NULL ){puts("Error: Out of memory...");return -1;}
				if(i==0)
				{
		...
		if(clust_cep==NULL){puts("Error: Out of memory...");return -1;}
		for(i=0;i<2;i++)
		{
			clust_cep[i] = new char*[n_clust[i]];
			if(clust_cep[i]==NULL ){puts("Error: Out of memory...");return -1;}
			int nsit_cur=0;
			for(j=0;j<n_clust[i];j++)
			{
				clust_cep[i][j] = new char[nsit_clust[i][j]+1];
				if(clust_cep[i][j]==NULL ){puts("Error: Out of memory...");return -1;}
				if(i==0)
				{
		...
		int ***cel_pv;
		cel_pv = new int**[2];				
		if(cel_pv==NULL){puts("Error: Out of memory...");return -1;}
		for(i=0;i<2;i++)
		{
			cel_pv[i] = new int*[n_clust[i]];
			if(cel_pv[i]==NULL ){puts("Error: Out of memory...");return -1;}
			int nsit_cur=0;
			for(j=0;j<n_clust[i];j++)
			{
				cel_pv[i][j] = new int[nsit_clust[i][j]+1];
				if(cel_pv[i][j]==NULL ){puts("Error: Out of memory...");return -1;}
				if(i==0)
				{
		...
		double ***pv;
		pv = new double**[2];				
		if(pv==NULL){puts("Error: Out of memory...");return -1;}
		for(i=0;i<2;i++)
		{
			pv[i] = new double*[n_clust[i]];
			if(pv[i]==NULL ){puts("Error: Out of memory...");return -1;}
			int nsit_cur=0;
			for(j=0;j<n_clust[i];j++)
			{
				pv[i][j] = new double[nsit_clust[i][j]+1];
				if(pv[i][j]==NULL ){puts("Error: Out of memory...");return -1;}
				if(i==0)
				{
					for(k=0;k<nsit_clust[i][j];k++)
					{
		...
		int ***shift;
		shift = new int**[2];				
		if(shift==NULL){puts("Error: Out of memory...");return -1;}
		for(i=0;i<2;i++)
		{
			shift[i] = new int*[n_clust[i]];
			if(shift[i]==NULL ){puts("Error: Out of memory...");return -1;}
			int nsit_cur=0;
			for(j=0;j<n_clust[i];j++)
			{
				shift[i][j] = new int[nsit_clust[i][j]];
				if(shift[i][j]==NULL ){puts("Error: Out of memory...");return -1;}
				if(i==0)
				{
		...
		//if(spacer_lenr==NULL ){puts("Error: Out of memory...");return -1;}
		int order[2][SEQLEN];
		//int **order;		
		//order = new int*[2];				
		//if(order==NULL){puts("Error: Out of memory...");return -1;}
		/*
		for(j=0;j<2;j++)
		{			
			order[j] = new int[n_clust[j]];
			if(order[j]==NULL){puts("Error: Out of memory...");return -1;}
		}*/
		//for(h=0;h<height;h++)					
